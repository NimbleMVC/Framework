...
/**
 * Value (date) is strictly after the given reference. The reference is either
 * a literal date string (e.g. "2026-01-01", "now") or another field name in
 * the validated data.
 */
class After extends AbstractRule implements ContextAwareRuleInterface
{
    ...
    public function validateInContext(mixed $value, string $field, array $data): void
    {
        ...
        if ($referenceTs === null || $valueTs <= $referenceTs) {
            $this->fail(str_replace(':date', (string)$this->reference, $this->msg('after')));
        }
    }
